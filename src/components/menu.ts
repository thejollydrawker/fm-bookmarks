...
class BookmarkMenu extends LitElement {
    ...
    render() {
        ...
        //close mobile menu if the screen size changes
        mql.onchange = (e) => {
            if (!e.matches) {
                this.isOpen = false;
            }
        ...
        return html`
            <nav>
                ...
                <div class="menu">
                    ...
                    <ul style="width: ${15 * this.menuItems.length}%;">
                        ${this.menuItems.map((item, index) => {
                            if(index !== this.menuItems.length-1) 
                                return html`<li><a class="menu-item">${item.title}</a></li>`; 
                            else return html `<bookmark-btn class="red" content="${item.title}"></bookmark-btn>`;
                            }
                        )}
                    </ul>
                </div>
                <div class="overlay" .hidden=${!this.isOpen}>
